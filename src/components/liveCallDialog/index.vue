    ...
    <div v-if="props.modelValue" class="popup">
      <slot name="modalContent">
        ...
        <!-- <van-image
          ...
          fit="cover"
        /> -->
        <div class="mengCeng" v-if="state.mengCeng"></div>
        <video
          v-if="props?.wsData?.videoStreamType === 2"
          :id="`videoElement`"
          ref="videoElement"
          class="videoPlayer"
          autoplay
            ...
            <div class="flex items-center">
              ...
              <div class="nameBox">
                <div class="nameBoxTop">
                  ...
                  <img
                    :src="getCountryImg(props?.wsData)"
                    alt=""
                    class="countryImg"
                  />
                </div>
        ...
        <div class="bottomBoxConnect" v-if="!state.isReactive">
          ...
          <div class="bottomBoxConnectFont" v-if="!state.isTimeOut">
            Invite you to video call…
          </div>
          <div class="bottomBoxConnectFont" v-else>
            Waiting for her to enter the call...
          </div>
          <!-- 挂断 -->
          <!-- <div
          ...
          <div class="bottomBoxInvite" v-if="!state.isReactive">
            ...
            <img v-else src="./assets/image@2x.webp" class="callingImg" />
          </div>
        ...
        <div class="bottomBoxPushStream" v-else>
          <div class="mineBox">
            <div id="local-video" class="mineVideo"></div>
          </div>
        </div>
...
watch(
  ...
  async (newValue) => {
    ...
    if (newValue) {
      ...
      if (!state.isReactive) {
        if (props?.wsData?.videoStreamType === 2 && flvjs.isSupported()) {
          var videoElement = document.getElementById(`videoElement`);

          flvPlayer.value = flvjs.createPlayer({
            type: "flv",
            url: convertRtmpToFlv(props?.wsData?.url),
            isLive: true,
            hasAudio: false,
          });
        ...
        if (props?.wsData?.videoStreamType === 1) {
          const video: any = document.getElementById("videoElements");
          const mediaUrl = props?.wsData?.url;
      ...
      toggleBodyScroll(false);
      clearInterval(mengcengTimer.value);
      if (flvPlayer.value) {
...
const handleClosePopup = async (type: number) => {
  showConfirmDialog({
    title: "",
...
const handleLoginRoom = async (
  ...
  await loginRoom(roomID, token, userID, userName).then(async (result) => {
    if (result) {
      ...
      mengcengTimer.value = setInterval(() => {
        state.mengCeng = false;
      }, 500);
      // let localStream: any;
      roomUpdate();
...
// 主动拨打逻辑
evenBus.on("activeCallLiveCall", async (data: any) => {
